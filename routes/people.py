...
def person_card(r):
    ...
    if dob:
        h += "<div class='row'><b>DOB</b> %s</div>" % dob
    if tob:
        h += "<div class='row'><b>TOB</b> %s</div>" % tob
    if loc:
        h += "<div class='row'><b>Loc</b> %s</div>" % loc
    if tags:
...
def list_people():
    ...
    if not rows:
        return page(h + '<div class="empty">No people yet. Tap + NEW to add one.</div>')
    for r in rows:
        h += person_card(r)
    return page(h)
...
(function(){
  ...
  function card(p){
    ...
    h += "<p class='card-title'><a href='/person/" + p.id + "'>" + p.name + "</a></p>";
...
def person_detail(pid):
    ...
    h += '<form method="post" action="/person/%d/delete" style="display:inline;" onsubmit="return confirm(\'Delete this person?\');">' % pid
...
def save_person(pid):
    ...
    if pid is None:
        ...
        run("""UPDATE people SET name=?, birth_date=?, birth_time=?,
               birth_place=?, manual_coords=?, broader_tags=?, multiple_tags=?,
               notes=? WHERE id=?""",
...
def person_form(row):
    ...
    if pid:
        link_section = (
            '<div class="section-label">Link Combination</div>'
            '<a class="btn btn-secondary btn-sm" href="/person/%d/link_combo">+ Link Combination</a>'
            '<div class="section-label">Link Research</div>'
            '<a class="btn btn-secondary btn-sm" href="/person/%d/link_research">+ Link Research</a>'
        ) % (pid, pid)
    ...
    h += '<label>Manual Coordinates (optional)</label><input type="text" name="manual" value="%s" placeholder="13.0827, 80.2707">' % (man or "")
    h += '<label>Tags <span class="muted">(Profession / Disease - comma separated)</span></label>'
    h += '<input type="text" name="broader_tags" value="%s" placeholder="cinema, sports">' % (tags or "")
    h += '<label>Short tags <span class="muted">(Family art / Dealer - comma separated)</span></label>'
    h += '<input type="text" name="multiple_tags" value="%s" placeholder="psycho movies, baseball">' % (stags or "")
    ...
    h += '<a class="btn btn-secondary" href="%s">CANCEL</a>%s</div>' % (back, del_btn)
    h += '</form></div>'
    return page(h)
...
def link_combo(pid):
    ...
    for cid, title, result in rows:
        h += "<div class='picker-row'><div class='nm'>%s</div><div class='ac'>" % (title or "(untitled)")
    ...
    if not rows:
        h += "<div class='empty'>No unlinked combinations.</div>"
    h += '<div style="margin-top:14px;"><a class="btn" href="/combination/new">+ NEW COMBINATION</a></div>'
    return page(h)
...
def link_research(pid):
    ...
    if not rows:
        h += "<div class='empty'>No unlinked research.</div>"
    h += '<div style="margin-top:14px;"><a class="btn" href="/research/new">+ NEW RESEARCH</a></div>'
    return page(h)
