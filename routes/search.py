...
def search():
    ...
    if term:
        ...
        ppl = q("""SELECT id, name, birth_place FROM people
                   WHERE name LIKE ? OR IFNULL(birth_place,'') LIKE ?
                      ...
                      OR IFNULL(multiple_tags,'') LIKE ? LIMIT 30""",
                (like, like, like, like))
        combos = q("""SELECT DISTINCT c.id, c.title, c.result FROM combinations c
                   ...
                   (like, like, like))
        res = q("""SELECT DISTINCT r.id, r.main_event, r.result FROM research r
        ...
        if res:
            ...
            for rid, ev, result in res:
                body += "<div class='row'>&bull; <a href='/research/%d'>%s</a>" % (rid, ev or "")
                if result: body += " &mdash; <span class='muted'>%s</span>" % result
                body += "</div>"
            body += "</div>"
    ...
    h += '<form method="get" class="search-bar"><input type="text" name="q" value="%s" placeholder="Search everything..." autofocus><button class="btn" type="submit">SEARCH</button></form>' % term
    h += body
    return page(h)
